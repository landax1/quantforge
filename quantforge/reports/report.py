...
def html_report(result: dict[str, Any], title: str, dataset_name: str) -> str:
    ...
    for group, gl in (("entry_long", "Long entry"), ("entry_short", "Short entry"),
        ...
        if conds:
            from quantforge.core.models import Condition
...
<html><head><meta charset="utf-8"><title>{escape(title)} — QuantForge report</title>
<style>
...
<h1>{escape(title)}</h1>
<p class="sub">Dataset: {escape(dataset_name)} · Generated by QuantForge (offline, deterministic)</p>
{''.join(rules)}
<h2>Performance</h2>
