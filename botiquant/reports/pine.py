...
# Botiquant indicator -> Pine expression template, by output name.
...
def export_pine(spec: StrategySpec, *, name: str = "QF Strategy",
    ...
    risk = spec.risk
    header = [f"// {name} — generado por Botiquant"]
    if symbol_hint:
        header.append(f"// Minada sobre {symbol_hint} {timeframe_hint}".rstrip())
    if metrics:
        header.append(
            f"// Backtest Botiquant: Net {metrics.get('net_profit_pct', 0):+.1f}% · "
